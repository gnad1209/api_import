...
const incommingDocumentSchema = new mongoose.Schema(
  {
    ...
    documentDate: { type: Date, set: (date) => moment(date, 'DD/MM/YYYY').format('YYYY/MM/DD') },
    receiveDate: { type: Date, set: (date) => moment(date, 'DD/MM/YYYY').format('YYYY/MM/DD') },
    toBookDate: { type: Date, set: (date) => moment(date, 'DD/MM/YYYY').format('YYYY/MM/DD') },
    deadline: { type: Date, set: (date) => moment(date, 'DD/MM/YYYY').format('YYYY/MM/DD') },
    stage: String,
    status: {
      type: Number,
      default: 1,
