...
const exportDataInZipFile = async (req, res, next) => {
  try {
    ...
    if (Array.isArray(receiveDateInput) && receiveDateInput.length === 2) {
      filter.receiveDate = {
        $gte: moment(receiveDateInput[0], 'YYYY/MM/DD').startOf('day').toDate(),
        $lte: moment(receiveDateInput[1], 'YYYY/MM/DD').endOf('day').toDate(),
      };
    }
